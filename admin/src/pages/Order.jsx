      ...
      <div className="space-y-6">
        {orders.length > 0 ? (
          orders.map((order, index) => (
            <div
              key={index}
              className="bg-white rounded-xl shadow-lg border border-gray-300 p-6 md:p-8 transition-all duration-300 hover:shadow-2xl"
            >
              {/* Nội dung đơn hàng */}
              <div className="p-6 bg-white rounded-lg shadow-lg grid grid-cols-4 gap-2 items-start">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-lg font-semibold text-gray-900">
                      ...
                      {/* {order.name} × {order.items[0].quantity} <span className="text-gray-500">({order.items[0].size})</span> */}
                    </p>
                ...
                <div className="flex justify-center items-center text-sm sm:text-[15px]">
                  {currency}{order.amount}
                </div>

                {/*Trạng thái đơn hàng */}
                <div className="flex flex-col items-center">
                  <select onChange={(event)=>statusHandler(event, order._id)} value={order.status} className="p-2 mt-2 border border-gray-400 rounded-lg px-3 py-1 text-sm bg-gray-100 hover:bg-gray-200 transition">
